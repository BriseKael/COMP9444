...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    ## using relu activation on the first layer
    logits = tf.matmul(X, w1) + b1
    preds = tf.nn.relu(logits)

    ## using softmax activation on the second layer
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    ##
    fc = tf.reshape(conv2, [-1, 28*28*convlayer_sizes[1]])
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(fc, Y, layersize=outputsize)
